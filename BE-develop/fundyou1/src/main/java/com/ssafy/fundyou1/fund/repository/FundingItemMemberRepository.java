...
public interface FundingItemMemberRepository extends JpaRepository<FundingItemMember, Long> {
    ...
    @Query(value = "select count(*) from (select distinct member_id from funding_item_member where funding_item_id = :fundingItemId) as attend_member_tmp ",nativeQuery = true)
