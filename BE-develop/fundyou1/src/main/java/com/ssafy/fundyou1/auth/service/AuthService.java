...
public class AuthService {
    ...
    @Transactional
    public BaseResponseBody reissue(TokenRequestDto tokenRequestDto) {
        // 1. Refresh Token 검증

        if (!tokenProvider.validateToken(tokenRequestDto.getRefreshToken())) {
            return new BaseResponseBody().of(500, "Refresh Token 이 유효하지 않습니다.", null);
        }
        ...
        if(!refreshToken.isPresent()){
            return new BaseResponseBody().of(500, "로그아웃된 사용자 입니다.", null);
        }


        // 4. Refresh Token 일치하는지 검사
        if (!refreshToken.get().getValue().equals(tokenRequestDto.getRefreshToken())) {
            return new BaseResponseBody().of(500, "토큰의 유저 정보가 일치하지 않습니다.", null);
        }
        ...
        // 6. 저장소 정보 업데이트
        RefreshToken newRefreshToken = refreshToken.get().updateValue(tokenDto.getRefreshToken());
        refreshTokenRepository.save(newRefreshToken);

        // 토큰 발급
        return new BaseResponseBody().of(200, "토큰 재발급 성공.", tokenDto);
    }
}
